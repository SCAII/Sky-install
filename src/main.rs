...
fn build_core(install_dir: &PathBuf) -> Result<(), Box<Error>> {
    ...
    if !scaii_install_dir.as_path().exists() {
        return Err(Box::new(InstallError::new(
            "scaii core has not been installed - run 'get-core' command first.",
        )));
    }
...
fn build_sky_rts(_install_dir: &PathBuf) -> Result<(), Box<Error>> {
    ...
    if !sky_rts_dir.as_path().exists() {
        return Err(Box::new(InstallError::new(
            "Sky-RTS has not been installed - run 'get-sky-rts' command first.",
        )));
    }
...
fn install_google_closure_library(
...
fn set_execute_permission(path_buf: &PathBuf) -> Result<(), Box<Error>> {
    if cfg!(target_os = "windows") {
        ...
        match result_string.as_str() {
            "" => Ok(()),
            ref x => Err(Box::new(InstallError::new(&format!(
                "couldnot set execute permission on {:?} : {}",
                path_buf, x
            )))),
        }
    }
